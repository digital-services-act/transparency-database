...
@section('content')
    ...
    <h1 class="ecl-page-header__title ecl-u-type-heading-1 ecl-u-mb-l">Statement of reason details: {{$statement->uuid}}
    </h1>
    ...
    <x-infoline title="Received" :content="$statement->created_at->format('Y-m-d H:i:s') . ' UTC'"></x-infoline>

    @if(!is_null($statement->decision_visibility))
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_DECISION_VISIBILITY"
        ...
        @if($statement->end_date_visibility_restriction)
            <x-infoline :title="StatementAlpha::LABEL_STATEMENT_END_DATE_VISIBILITY_RESTRICTION"
    ...
    @if(!is_null($statement->decision_monetary))
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_DECISION_MONETARY"
            :content="StatementAlpha::DECISION_MONETARIES[$statement->decision_monetary]"></x-infoline>
        ...
        @if($statement->end_date_monetary_restriction)
            <x-infoline :title="StatementAlpha::LABEL_STATEMENT_END_DATE_MONETARY_RESTRICTION"
                :content="$statement->end_date_monetary_restriction->format('Y-m-d')"></x-infoline>
        @endif
    @endif

    @if(!is_null($statement->decision_provision))
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_DECISION_PROVISION"
            :content="StatementAlpha::DECISION_PROVISIONS[$statement->decision_provision]"></x-infoline>

        @if($statement->end_date_service_restriction)
            <x-infoline :title="StatementAlpha::LABEL_STATEMENT_END_DATE_SERVICE_RESTRICTION"
    ...
    @if(!is_null($statement->decision_account))
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_DECISION_ACCOUNT"
            :content="StatementAlpha::DECISION_ACCOUNTS[$statement->decision_account]"></x-infoline>

        @if($statement->end_date_account_restriction)
            <x-infoline :title="StatementAlpha::LABEL_STATEMENT_END_DATE_ACCOUNT_RESTRICTION"
                :content="$statement->end_date_account_restriction->format('Y-m-d')"></x-infoline>
        @endif
    @endif

    @if(!is_null($statement->account_type))
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_ACCOUNT_TYPE"
            :content="StatementAlpha::ACCOUNT_TYPES[$statement->account_type]"></x-infoline>
    @endif

    <x-infoline :title="StatementAlpha::LABEL_STATEMENT_DECISION_FACTS" :content="$statement->decision_facts"></x-infoline>

    <x-infoline :title="StatementAlpha::LABEL_STATEMENT_DECISION_GROUND"
        :content="StatementAlpha::DECISION_GROUNDS[$statement->decision_ground]"></x-infoline>

    @if(!is_null($statement->decision_account))
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_DECISION_GROUND_REFERENCE_URL"
            :content="$statement->decision_ground_reference_url"></x-infoline>
    @endif

    @if($statement->decision_ground == 'DECISION_GROUND_ILLEGAL_CONTENT')
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_ILLEGAL_CONTENT_GROUND"
            :content="$statement->illegal_content_legal_ground"></x-infoline>
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_ILLEGAL_CONTENT_EXPLANATION"
    ...
    @if($statement->decision_ground == 'DECISION_GROUND_INCOMPATIBLE_CONTENT')
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_INCOMPATIBLE_CONTENT_GROUND"
            :content="$statement->incompatible_content_ground"></x-infoline>
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_INCOMPATIBLE_CONTENT_EXPLANATION"
            :content="$statement->incompatible_content_explanation"></x-infoline>
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_INCOMPATIBLE_CONTENT_ILLEGAL"
            :content="is_null($statement->incompatible_content_illegal) ? 'N/A' : $statement->incompatible_content_illegal">
        </x-infoline>

    @endif

    <x-infoline :title="StatementAlpha::LABEL_STATEMENT_TERRITORIAL_SCOPE"
        content="{{ implode(', ', $statement_territorial_scope_country_names) }}"></x-infoline>

    <x-infoline :title="StatementAlpha::LABEL_STATEMENT_CONTENT_TYPE"
    ...
    <x-infoline :title="StatementAlpha::LABEL_STATEMENT_CONTENT_DATE"
        :content="$statement->content_date->format('Y-m-d')"></x-infoline>

    @if($statement_content_language)
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_CONTENT_LANGUAGE"
    ...
    <x-infoline :title="StatementAlpha::LABEL_STATEMENT_SOURCE_TYPE"
        :content="StatementAlpha::SOURCE_TYPES[$statement->source_type]"></x-infoline>

    @if($statement->source_type !== 'SOURCE_VOLUNTARY')
        <x-infoline :title="StatementAlpha::LABEL_STATEMENT_SOURCE_IDENTITY"
            :content="$statement->source_identity"></x-infoline>
    @endif

    <x-infoline :title="StatementAlpha::LABEL_STATEMENT_AUTOMATED_DETECTION"
        :content="$statement->automated_detection"></x-infoline>
    <x-infoline :title="StatementAlpha::LABEL_STATEMENT_AUTOMATED_DECISION"
        :content="StatementAlpha::AUTOMATED_DECISIONS[$statement->automated_decision]"></x-infoline>

    <x-infoline :title="StatementAlpha::LABEL_STATEMENT_APPLICATION_DATE"

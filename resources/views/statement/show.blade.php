...
@section('content')
    ...
    <x-infoline :title="Statement::LABEL_STATEMENT_COUNTRY_LIST" content="{{ implode(', ', $statement->getCountriesListNames()) }}"></x-infoline>

    <x-infoline :title="Statement::LABEL_STATEMENT_CONTENT_TYPE" :content="Statement::CONTENT_TYPES[$statement->content_type]"></x-infoline>
    @if($statement->content_type == 'CONTENT_TYPE_OTHER')
        <x-infoline title="" :content="$statement->content_type_other"></x-infoline>
    @endif


    <x-infoline :title="Statement::LABEL_STATEMENT_CATEGORY" :content="Statement::STATEMENT_CATEGORIES[$statement->category]"></x-infoline>

    <x-infoline :title="Statement::LABEL_STATEMENT_URL" :content="$statement->url"></x-infoline>

    <x-infoline :title="Statement::LABEL_STATEMENT_SOURCE_TYPE" :content="Statement::SOURCE_TYPES[$statement->source_type]"></x-infoline>
    ...
    <x-infoline :title="Statement::LABEL_STATEMENT_AUTOMATED_DETECTION" :content="$statement->automated_detection"></x-infoline>
    <x-infoline :title="Statement::LABEL_STATEMENT_AUTOMATED_DECISION" :content="$statement->automated_decision"></x-infoline>

    @if($statement->start_date)
        <x-infoline :title="Statement::LABEL_STATEMENT_START_DATE" :content="$statement->start_date->format('d-m-Y')"></x-infoline>
    @endif

    @if($statement->end_date)
        <x-infoline :title="Statement::LABEL_STATEMENT_END_DATE" :content="$statement->end_date->format('d-m-Y')"></x-infoline>
    @else

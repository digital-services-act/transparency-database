...
On 8th of April 2024, the transparency database (TDB) team is updating the structure of the daily dumps CSV files available in the [data download](/data-download) section. This change aims at improving the daily dump CSV creation to speed it up and make it more computationally efficient. This will enable to publish the CSV files in a quick and timely manner even with the current high daily volume of statements of reasons (SoRs) submitted to the TDB, which is expected to further increase in the coming months when small platforms will onboard.

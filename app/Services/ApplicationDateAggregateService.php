...
use App\Models\ApplicationDateAggregate;
use App\Models\Platform;
use Exception;
use Illuminate\Support\Carbon;
use Illuminate\Support\Facades\DB;

class ApplicationDateAggregateService
{
    /**
     * @throws Exception
     */
    public function compileDayTotals(Carbon $date)
    {
        ...
        // Delete any existing
        ApplicationDateAggregate::query()->where('date', $date)->delete();
        ...
        $results = DB::table('statements')
            ...
            ->where('application_date', '<=', $end)->get();

        $platforms = Platform::all()->pluck('name', 'id')->toArray();

        foreach ($results as $result)
        {
            ApplicationDateAggregate::create([

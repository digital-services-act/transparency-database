    ...
    public function store(StatementStoreRequest $request): JsonResponse
    {
        ...
        try {
            ...
            if (
                ...
                str_contains($queryException->getMessage(),
                    "UNIQUE constraint failed: statements.platform_id, statements.puid") // sqlite
            ) {
                $errors = [
                    'puid' => [
                ...
                $existing = Statement::query()->where('puid', $validated['puid'])->where('platform_id',
                    $validated['platform_id'])->first();
                if ($existing) {

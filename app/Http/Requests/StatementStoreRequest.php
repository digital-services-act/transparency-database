...
class StatementStoreRequest extends FormRequest
{
    ...
    {
        return [
            'decision_visibility' => [ self::in(array_keys(Statement::DECISION_VISIBILITIES)),'required_without_all:decision_monetary,decision_provision,decision_account'],
            ...
            'decision_monetary' => [ self::in(array_keys(Statement::DECISION_MONETARIES)),'required_without_all:decision_visibility,decision_provision,decision_account'],
            ...
            'decision_provision' => [ self::in(array_keys(Statement::DECISION_PROVISIONS)),'required_without_all:decision_visibility,decision_monetary,decision_account'],
            'decision_account' => [ self::in(array_keys(Statement::DECISION_ACCOUNTS)),'required_without_all:decision_visibility,decision_monetary,decision_provision'],


            'decision_ground' => ['required', self::in(array_keys(Statement::DECISION_GROUNDS))],
            ...
            'incompatible_content_illegal' => ['required_if:decision_ground,DECISION_GROUND_INCOMPATIBLE_CONTENT', self::in(Statement::INCOMPATIBLE_CONTENT_ILLEGALS),'exclude_unless:decision_ground,DECISION_GROUND_INCOMPATIBLE_CONTENT'],

            'content_type' => ['required', self::in(array_keys(Statement::CONTENT_TYPES))],
            'content_type_other' => ['required_if:content_type,CONTENT_TYPE_OTHER','exclude_unless:content_type,CONTENT_TYPE_OTHER'],

            'category' => ['required', self::in(array_keys(Statement::STATEMENT_CATEGORIES))],
            'countries_list' => ['array', 'nullable', self::in(Statement::EUROPEAN_COUNTRY_CODES)],
            'start_date' => ['required', 'date'],
            'end_date' => ['date', 'nullable','after_or_equal:start_date'],
            'decision_facts' => ['required'],
            'source_type' => ['required', self::in(array_keys(Statement::SOURCE_TYPES))],
            'source' => ['required_unless:source_type,SOURCE_VOLUNTARY','exclude_if:source_type,SOURCE_VOLUNTARY'],
            'automated_detection' => ['required', self::in(Statement::AUTOMATED_DETECTIONS)],
            'automated_decision' => ['required', self::in(Statement::AUTOMATED_DECISIONS)],
//            'automated_takedown' => ['required', self::in(Statement::AUTOMATED_TAKEDOWNS)],
            'url' => ['url','required'],
        ];
    }

    private static function in($array): string
    {
